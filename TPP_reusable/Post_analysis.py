...
def site_based(input):
    ...
    df=df.fillna('')
    df['All_PTMs'] = df['PTM']
    df['All_PTM_scores'] = df['PTM_score']
    df['All_PTM_positions'] = df['PTM_positions']
    df['PTM'] = df['PTM'].str.split(';')
    df['PTM_score'] = df['PTM_score'].str.split(';')
    df['PTM_positions'] = df['PTM_positions'].str.split(';')
...
def model_FLR_new(file):
    ...
    df['PTM_final_prob'] = round(df['Score'] * df['PTM_score'],2)
...
def model_FLR_filter(file):
    ...
    if "pG" in file:
        df = df[df.Peptide.str.count('S|T|Y|G') > df.Peptide_mod.str.count("Phosphorylation")]
    elif "pL" in file:
        df = df[df.Peptide.str.count('S|T|Y|L') > df.Peptide_mod.str.count("Phosphorylation")]
    elif "pD" in file:
        df = df[df.Peptide.str.count('S|T|Y|D') > df.Peptide_mod.str.count("Phosphorylation")]
    elif "pE" in file:
        df = df[df.Peptide.str.count('S|T|Y|E') > df.Peptide_mod.str.count("Phosphorylation")]
    elif "pP" in file:
        df = df[df.Peptide.str.count('S|T|Y|P') > df.Peptide_mod.str.count("Phosphorylation")]
    else:
        df = df[df.Peptide.str.count('S|T|Y|A')>df.Peptide_mod.str.count("Phosphorylation")]
    df = df.reset_index(drop=True)
...
def PTM_sort_FLR(input):
    ...
    df = df[~df.Protein.str.contains("CONTAM",na=False)]
    df = df.reset_index(drop=True)
    df = df.sort_values(by=(['PTM_score']), ascending=[False])
    df = df.reset_index(drop=True)
    df['Count'] = (df.index) + 1
